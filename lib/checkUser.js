...
export const checkUser = async () => {
  ...
  try {
    ...
    const loggedInUser = await db.user.upsert({
      // 1. Look up the user by their unique email address instead
      where: {
        email: user.emailAddresses[0].emailAddress,
      },

      // 2. If the email exists, just link their clerkUserId and sync fields
      update: {
        currentPlan,
        clerkUserId: user.id, // Keeps the Clerk link intact if it changed
        name: `${user.firstName || ""} ${user.lastName || ""}`.trim(),
        imageUrl: user.imageUrl,
      },

      // 3. If the email doesn't exist anywhere, create a brand new user row safely
      create: {
        clerkUserId: user.id,
        name: `${user.firstName || ""} ${user.lastName || ""}`.trim(),
        imageUrl: user.imageUrl,
