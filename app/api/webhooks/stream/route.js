...
Analyze the candidate's performance. Respond ONLY with a valid JSON object, no markdown, no backticks, no explanation:
{
  ...
  "strengths": ["First strength text here", "Second strength text here", "Third strength text here"],
  "improvements": ["First improvement text here", "Second improvement text here", "Third improvement text here"],
      ...
      await db.$transaction([
        db.feedback.upsert({
          ...
          update: {
            ...
            improvements: feedbackData.improvements,
            overallRating: feedbackData.overallRating,
          }, // already exists — no-op, keep the original
        }),
        db.booking.update({
          where: { id: booking.id },
